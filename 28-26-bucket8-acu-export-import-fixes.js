...
(function(){
  ...
  const ACU_NOTE_IMAGE_PREFIX = "mtc_point_note_image_";
  ...
  function importHistoryPrefixes(){
    ...
    return personalDataPrefixes().filter(prefix => prefix !== PHARMA_IMAGE_PREFIX && prefix !== ACU_IMAGE_PREFIX && prefix !== ACU_NOTE_IMAGE_PREFIX);
  }
  ...
  window.exportPersonalNotes = function(){
    (async () => {
      ...
      acupuncture:{
        ...
        note_images:safeLocalStorageEntries(ACU_NOTE_IMAGE_PREFIX)
      },
      pharmacology:{
...
/* === Éditeur local complet des bases ACU / PHARMA === */
(function(){
  "use strict";

  const STORAGE_KEY = "mtc_database_overrides_v1";
  const FILE_TYPE = "connections-mtc-editable-database";
  ...
  const ADMIN_MODE = resolveAdminMode();
  window.MTC_DATABASE_ADMIN_MODE = ADMIN_MODE;
  document.documentElement.classList.toggle("mtc-database-admin",ADMIN_MODE);

  const LEGACY_PREFIXES = {
    acupuncture:{
      notes:"mtc_point_note_",
      esprit:"mtc_point_esprit_",
      associations:"mtc_point_associations_",
      vs:"mtc_point_vs_",
      precaution:"mtc_point_precaution_",
      synthese:"mtc_acu_point_synthese_"
    },
    pharmacology:{
      hanzi:"mtc_pharma_herb_hanzi_",
      esprit:"mtc_pharma_herb_esprit_",
      notes:"mtc_pharma_herb_notes_",
      associations:"mtc_pharma_herb_associations_",
      formules:"mtc_pharma_herb_formules_",
      vs:"mtc_pharma_herb_vs_",
      precaution:"mtc_pharma_herb_precaution_",
      synonymes:"mtc_pharma_herb_synonymes_",
      synthese:"mtc_pharma_herb_synthese_",
      ingredients:"mtc_pharma_herb_ingredients_",
      recherches_modernes:"mtc_pharma_herb_recherches_modernes_",
      indications:"mtc_pharma_herb_indications_",
      contre_indications:"mtc_pharma_herb_contre_indications_",
      preparation:"mtc_pharma_herb_preparation_"
    }
  };

  const FIELD_LABELS = {
    pinyin:"Pinyin", pinyinSansTons:"Pinyin sans tons", hanzi:"Hanzi", nom:"Nom français",
    nom_francais:"Nom français", nom_complet:"Nom complet", classe:"Classe", classCode:"Code de classe",
    nature:"Nature", saveur:"Saveur", tropisme:"Tropisme", posologie:"Posologie", actions:"Actions",
    indications:"Indications", contre_indications:"Contre-indications", contraindications:"Contre-indications",
    precaution:"Précaution", precautions:"Précautions", associations:"Associations", association:"Association",
    vs:"VS.", comparaison:"Comparaison", formules:"Formules", esprit:"Esprit", synthese:"Synthèse",
    ingredients:"Ingrédients", recherches_modernes:"Recherches modernes", preparation:"Préparation",
    synonymes:"Synonymes", notes:"Notes", localisation:"Localisation",
    methode_localisation:"Méthode de localisation", methode_travail:"Méthode de travail",
    categories_du_point:"Catégories du point", correspondances:"Correspondances", canal:"Canal",
    prioritaire:"Prioritaire", toxicite:"Toxicité", toxicité:"Toxicité", code:"Code"
  };

  const FIELD_ORDER = {
    acupuncture:[
      "pinyin","hanzi","nom_francais","nom_complet","canal","localisation","methode_localisation",
      "methode_travail","categories_du_point","correspondances","actions","indications","associations",
      "vs","precaution","esprit","synthese","notes"
    ],
    pharmacology:[
      "pinyin","pinyinSansTons","hanzi","nom","classe","classCode","nature","saveur","tropisme",
      "posologie","actions","indications","contre_indications","precaution","associations","vs",
      "formules","esprit","synthese","ingredients","recherches_modernes","preparation","synonymes",
      "notes","prioritaire"
    ]
  };
  ...
  function pharmaArray(){
    return Array.isArray(window.PHARMA_HERBS) ? window.PHARMA_HERBS : [];
  }

  function pharmaMap(){
    const out = {};
    pharmaArray().forEach(item => { if(item && item.id) out[item.id] = item; });
    return out;
  }

  /* Les snapshots officiels servent à ne stocker que les différences locales. */
  const OFFICIAL_ACU = clone(acuMap());
  const OFFICIAL_PHARMA = clone(pharmaMap());

  Object.values(OFFICIAL_ACU).forEach(record => {
    if(record && !Object.prototype.hasOwnProperty.call(record,"associations")) record.associations = "";
  });
  Object.values(OFFICIAL_PHARMA).forEach(record => {
    if(record && !Object.prototype.hasOwnProperty.call(record,"associations")) record.associations = "";
  });
  Object.values(acuMap()).forEach(record => {
    if(record && !Object.prototype.hasOwnProperty.call(record,"associations")) record.associations = "";
  });
  pharmaArray().forEach(record => {
    if(record && !Object.prototype.hasOwnProperty.call(record,"associations")) record.associations = "";
  });
  ...
  function readOverrides(){
    try{
      ...
      return {
        acupuncture:parsed && parsed.acupuncture && typeof parsed.acupuncture === "object" ? parsed.acupuncture : {},
        pharmacology:parsed && parsed.pharmacology && typeof parsed.pharmacology === "object" ? parsed.pharmacology : {}
      };
    }catch(error){
      return {acupuncture:{}, pharmacology:{}};
    }
  }
  ...
  function writeOverrides(){
    try{
      ...
      const message = document.getElementById("message");
      if(message) message.textContent = "Corrections trop volumineuses pour le stockage local.";
      return false;
    }
  }

  function emptyEquivalent(value){
    return value === undefined || value === null || value === "" || (Array.isArray(value) && value.length === 0);
  }

  function equalValue(a,b){
    if(emptyEquivalent(a) && emptyEquivalent(b)) return true;
  ...
  function applyPatch(target, patch){
    ...
    Object.entries(patch).forEach(([key,value]) => {
      if(key === "id" || key === "point") return;
      target[key] = clone(value);
    });
    if(!Object.prototype.hasOwnProperty.call(target,"associations")) target.associations = "";
  }

  function syncLegacyField(domain, id, field, value, active){
    const prefix = LEGACY_PREFIXES[domain] && LEGACY_PREFIXES[domain][field];
    if(!prefix) return;
    try{
      const key = prefix + id;
      if(active){
  ...
  function hydrateLegacy(domain, records){
    const mapping = LEGACY_PREFIXES[domain] || {};
    Object.entries(records).forEach(([id,record]) => {
      Object.entries(mapping).forEach(([field,prefix]) => {
        try{
          const value = localStorage.getItem(prefix + id);
          if(value !== null) record[field] = value;
        }catch(error){}
      });
      if(!Object.prototype.hasOwnProperty.call(record,"associations")) record.associations = "";
    });
  }

  function applyAllOverrides(){
    const acu = acuMap();
    const pharma = pharmaMap();
    hydrateLegacy("acupuncture", acu);
    hydrateLegacy("pharmacology", pharma);
    Object.entries(overrides.acupuncture || {}).forEach(([id,patch]) => {
      if(acu[id]){
        applyPatch(acu[id], patch);
        Object.entries(patch || {}).forEach(([field,value]) => syncLegacyField("acupuncture",id,field,value,true));
      }
    });
    Object.entries(overrides.pharmacology || {}).forEach(([id,patch]) => {
      if(pharma[id]){
        applyPatch(pharma[id], patch);
        Object.entries(patch || {}).forEach(([field,value]) => syncLegacyField("pharmacology",id,field,value,true));
      }
    });
    rebuildPharmaClassMetadata();
  }

  function rebuildPharmaClassMetadata(){
    if(!Array.isArray(window.PHARMA_CLASSES)) return;
    window.PHARMA_CLASSES.forEach(item => {
      const records = pharmaArray().filter(herb => herb && herb.classCode === item.code);
      item.codes = records.map(herb => herb.id);
      item.count = records.length;
      item.prioritaires = records.filter(herb => Boolean(herb.prioritaire)).length;
    });
  }
  ...
  function diffRecord(base, edited){
    ...
    keys.forEach(key => {
      if(key === "id" || key === "point" || key.startsWith("_")) return;
      if(!equalValue(base ? base[key] : undefined, edited ? edited[key] : undefined)) patch[key] = clone(edited[key]);
    });
    return patch;
  }

  function recordMaps(domain){
    if(domain === "pharmacology") return {official:OFFICIAL_PHARMA, current:pharmaMap()};
    return {official:OFFICIAL_ACU, current:acuMap()};
  }

  function saveRecord(domain, id, edited, importedFields){
    const maps = recordMaps(domain);
    const target = maps.current[id];
    const base = maps.official[id];
    if(!target || !base) return false;
    const next = Object.assign({}, cleanRecord(target), cleanRecord(edited || {}));
    if(!Object.prototype.hasOwnProperty.call(next,"associations")) next.associations = "";
    const patch = diffRecord(base, next);
    if(Object.keys(patch).length) overrides[domain][id] = patch;
    else delete overrides[domain][id];
    applyPatch(target, next);
    const fields = importedFields || Object.keys(next);
    fields.forEach(field => syncLegacyField(domain,id,field,next[field],Object.prototype.hasOwnProperty.call(patch,field)));
    writeOverrides();
    rebuildPharmaClassMetadata();
    document.dispatchEvent(new CustomEvent("mtc-database-edited", {detail:{domain,id}}));
    if(domain === "pharmacology") document.dispatchEvent(new CustomEvent("pharma-herb-edited", {detail:{herbId:id,field:"database"}}));
    else document.dispatchEvent(new CustomEvent("acu-point-edited", {detail:{point:id,field:"database"}}));
    return true;
  }

  function canonicalKeys(domain, record){
    const ordered = FIELD_ORDER[domain] || [];
    const extras = Object.keys(record || {}).filter(key => !key.startsWith("_") && key !== "id" && key !== "point" && !ordered.includes(key)).sort();
    return [...ordered, ...extras].filter((key,index,list) => list.indexOf(key) === index);
  }

  function fieldKind(value){
    if(Array.isArray(value)){
      return value.every(item => item === null || ["string","number","boolean"].includes(typeof item)) ? "array" : "json";
    }
  ...
  function fieldText(value, kind){
    if(kind === "array") return (value || []).map(item => String(item ?? "")).join("\n");
  ...
  function renderField(key, value){
    ...
    if(kind === "boolean"){
      return `<label class="mtc-db-field"><span>${esc(label)}</span><select data-mtc-db-field="${attr(key)}" data-mtc-db-kind="boolean"><option value="true" ${value ? "selected" : ""}>Oui</option><option value="false" ${!value ? "selected" : ""}>Non</option></select></label>`;
    }
    const text = fieldText(value,kind);
    const short = kind !== "json" && kind !== "array" && text.length < 90 && !text.includes("\n");
    if(short){
      return `<label class="mtc-db-field"><span>${esc(label)}</span><input type="${kind === "number" ? "number" : "text"}" value="${attr(text)}" data-mtc-db-field="${attr(key)}" data-mtc-db-kind="${attr(kind)}"></label>`;
    }
    const rows = Math.min(16, Math.max(3, text.split("\n").length + 1));
    return `<label class="mtc-db-field mtc-db-field-wide"><span>${esc(label)}</span><textarea rows="${rows}" data-mtc-db-field="${attr(key)}" data-mtc-db-kind="${attr(kind)}">${esc(text)}</textarea></label>`;
  }

  window.renderMtcFullRecordEditor = function(domain,id,record){
    if(!ADMIN_MODE || !record) return "";
    if(!Object.prototype.hasOwnProperty.call(record,"associations")) record.associations = "";
    const fields = canonicalKeys(domain,record).map(key => renderField(key, record[key])).join("");
    const label = domain === "pharmacology" ? "substance" : "point";
    return `
      <section class="mtc-db-record-editor" data-mtc-db-editor="${attr(domain)}" data-mtc-db-record-id="${attr(id)}">
        <button type="button" class="mtc-db-record-editor-toggle" aria-expanded="false">✎ Corriger la fiche</button>
        <div class="mtc-db-record-editor-body" hidden>
          <p class="mtc-db-record-editor-note">Mode administrateur : corrige directement les champs de ce ${label}. Il n’y a pas de bouton Enregistrer : chaque champ est sauvegardé automatiquement quand tu le quittes. L’identifiant <strong>${esc(id)}</strong> reste verrouillé.</p>
          <p class="mtc-db-autosave-status" role="status" aria-live="polite">Prêt — enregistrement automatique</p>
          <div class="mtc-db-record-editor-grid">${fields}</div>
        </div>
      </section>`;
  };

  function parseField(element){
    const kind = element.dataset.mtcDbKind || "string";
    const value = element.value;
    if(kind === "boolean") return value === "true";
    if(kind === "number") return value === "" ? "" : Number(value);
    if(kind === "array") return value.split(/\n/).map(item => item.trim()).filter(Boolean);
    if(kind === "json"){
      try{return JSON.parse(value || "{}");}catch(error){throw new Error(`JSON invalide dans « ${element.dataset.mtcDbField} »`);}
    }
    return value.replace(/\r\n/g,"\n").replace(/\r/g,"\n");
  }

  window.bindMtcFullRecordEditor = function(root,onSaved){
    if(!ADMIN_MODE) return;
    const scope = root || document;
    scope.querySelectorAll("[data-mtc-db-editor]").forEach(editor => {
      if(editor.dataset.mtcDbBound === "1") return;
      editor.dataset.mtcDbBound = "1";
      const toggle = editor.querySelector(".mtc-db-record-editor-toggle");
      const body = editor.querySelector(".mtc-db-record-editor-body");
      const status = editor.querySelector(".mtc-db-autosave-status");
      let dirty = false;
      ...
      const commitInput = input => {
        const domain = editor.dataset.mtcDbEditor;
        const id = editor.dataset.mtcDbRecordId;
        const maps = recordMaps(domain);
        const current = maps.current[id];
        if(!current || !input) return true;
        const key = input.dataset.mtcDbField;
        if(!key) return true;
        let value;
        try{
          value = parseField(input);
        }catch(error){
          input.classList.add("mtc-db-field-error");
          setStatus(error.message || "Valeur invalide — non enregistrée","error");
          return false;
        }
        input.classList.remove("mtc-db-field-error");
        if(equalValue(current[key],value)){
          setStatus("Aucune modification à enregistrer","idle");
          return true;
        }
        const edited = cleanRecord(current);
        edited[key] = value;
        if(saveRecord(domain,id,edited,[key])){
          dirty = true;
          input.dataset.mtcDbSavedValue = fieldText(value,input.dataset.mtcDbKind || fieldKind(value));
          setStatus(`✓ ${FIELD_LABELS[key] || key.replace(/_/g," ")} enregistré automatiquement`,"saved");
          return true;
        }
        setStatus("Enregistrement local impossible","error");
        return false;
      };

      const commitAll = () => {
        let ok = true;
        editor.querySelectorAll("[data-mtc-db-field]").forEach(input => {
          if(!commitInput(input)) ok = false;
        });
        return ok;
      };

      const setOpen = open => {
        if(!open && !commitAll()) return;
        body.hidden = !open;
        toggle.setAttribute("aria-expanded",open ? "true" : "false");
        toggle.textContent = open ? "Fermer les corrections" : "✎ Corriger la fiche";
        if(!open && dirty && typeof onSaved === "function"){
          dirty = false;
          onSaved();
        }
      };
      ...
      editor.querySelectorAll("[data-mtc-db-field]").forEach(input => {
        input.dataset.mtcDbSavedValue = input.value;
        input.addEventListener("input",() => {
          input.classList.remove("mtc-db-field-error");
          setStatus("Modification en cours… elle sera enregistrée quand tu quitteras le champ.","editing");
        });
        input.addEventListener("blur",() => commitInput(input));
        if(input.tagName === "SELECT") input.addEventListener("change",() => commitInput(input));
  ...
  function effectiveAcuExport(){
    const out = {};
    Object.entries(acuMap()).forEach(([id,record]) => {
      const clean = cleanRecord(record);
      clean.point = clean.point || id;
      if(!Object.prototype.hasOwnProperty.call(clean,"associations")) clean.associations = "";
      out[id] = clean;
    });
    return out;
  }

  function effectivePharmaExport(){
    const out = {};
    pharmaArray().forEach(record => {
  ...
  window.exportEditableDatabase = function(){
    const stamp = new Date().toISOString().slice(0,10);
    downloadJson(`connections-mtc-base-editable-${stamp}.json`,{
      app:"Connections MTC",
      type:FILE_TYPE,
      version:1,
      exportedAt:new Date().toISOString(),
      instructions:[
        "Modifier uniquement les valeurs des champs.",
        "Ne pas renommer les identifiants des points ou substances.",
        "Un champ peut être vidé avec une chaîne vide.",
        "Réimporter ce fichier depuis le bouton Base locale du jeu."
      ],
      acupuncture:effectiveAcuExport(),
      pharmacology:effectivePharmaExport()
    });
  };

  function importedSection(data,names){
  ...
  function applyImportedSection(domain,section){
    const maps = recordMaps(domain);
    let imported = 0;
    let unknown = 0;
    Object.entries(section || {}).forEach(([id,record]) => {
      if(!maps.current[id] || !record || typeof record !== "object"){
        unknown++;
        return;
      }
      const fields = Object.keys(record).filter(key => key !== "id" && key !== "point" && !key.startsWith("_"));
      const candidate = Object.assign({}, cleanRecord(maps.current[id]), cleanRecord(record));
      if(!Object.prototype.hasOwnProperty.call(candidate,"associations")) candidate.associations = "";
      const patch = diffRecord(maps.official[id], candidate);
      if(Object.keys(patch).length) overrides[domain][id] = patch;
      else delete overrides[domain][id];
      applyPatch(maps.current[id], candidate);
      fields.forEach(field => syncLegacyField(domain,id,field,candidate[field],Object.prototype.hasOwnProperty.call(patch,field)));
      imported++;
    });
    return {imported,unknown};
  }

  window.importDatabaseCorrectionsFromFile = function(input){
    const file = input && input.files && input.files[0];
    if(!file) return;
    const reader = new FileReader();
    reader.onload = () => {
      try{
        ...
        const acuSection = importedSection(data,["acupuncture","points","acu"]);
        const pharmaSection = importedSection(data,["pharmacology","substances","pharma"]);
        if(!Object.keys(acuSection).length && !Object.keys(pharmaSection).length){
          throw new Error("Le fichier ne contient ni section acupuncture ni section pharmacology.");
        }
        const acuResult = applyImportedSection("acupuncture",acuSection);
        const pharmaResult = applyImportedSection("pharmacology",pharmaSection);
        writeOverrides();
        rebuildPharmaClassMetadata();
        document.dispatchEvent(new CustomEvent("mtc-database-edited", {detail:{domain:"all",id:"*"}}));
        document.dispatchEvent(new CustomEvent("pharma-herb-edited", {detail:{herbId:"*",field:"database-import"}}));
        document.dispatchEvent(new CustomEvent("acu-point-edited", {detail:{point:"*",field:"database-import"}}));
        closeDatabaseEditorDialog();
        const message = document.getElementById("message");
        if(message) message.textContent = `Base locale importée : ${acuResult.imported} points, ${pharmaResult.imported} substances.`;
        if(typeof window.renderComparisonPanelIfOpen === "function") window.renderComparisonPanelIfOpen();
        if(typeof window.renderAdvancedSearchPanelIfOpen === "function") window.renderAdvancedSearchPanelIfOpen();
        if(typeof window.refreshCurrentPointPanel === "function") window.refreshCurrentPointPanel();
        if(typeof window.refreshCurrentPharmaHerbPanel === "function") window.refreshCurrentPharmaHerbPanel();
      }catch(error){
        alert("Import de la base impossible : " + (error.message || error));
      }finally{
        input.value = "";
      }
    };
    reader.onerror = () => { input.value = ""; alert("Lecture du fichier impossible."); };
    reader.readAsText(file,"utf-8");
  };

  function ensureDatabaseDialog(){
    let overlay = document.getElementById("mtcDatabaseEditorOverlay");
    if(overlay) return overlay;
    overlay = document.createElement("div");
    overlay.id = "mtcDatabaseEditorOverlay";
    overlay.className = "mtc-db-overlay";
    overlay.hidden = true;
    overlay.innerHTML = `
      <section class="mtc-db-dialog" role="dialog" aria-modal="true" aria-labelledby="mtcDbDialogTitle">
        <button type="button" class="mtc-db-dialog-close" aria-label="Fermer">×</button>
        <h2 id="mtcDbDialogTitle">Corrections administrateur</h2>
        <p>Exporte les corrections pour les archiver ou les publier, ou réimporte un fichier corrigé. Les changements effectués dans les fiches sont déjà enregistrés automatiquement dans ce navigateur.</p>
        <p><strong>Les identifiants</strong> des points et substances ne doivent pas être renommés.</p>
        <div class="mtc-db-dialog-actions">
          <button type="button" data-mtc-db-export>Exporter la base éditable</button>
          <button type="button" data-mtc-db-import>Importer le fichier corrigé</button>
        </div>
        <input type="file" accept="application/json,.json" hidden data-mtc-db-file>
      </section>`;
    document.body.appendChild(overlay);
    overlay.querySelector(".mtc-db-dialog-close").addEventListener("click",closeDatabaseEditorDialog);
    overlay.addEventListener("click",event => { if(event.target === overlay) closeDatabaseEditorDialog(); });
    overlay.querySelector("[data-mtc-db-export]").addEventListener("click",window.exportEditableDatabase);
    const fileInput = overlay.querySelector("[data-mtc-db-file]");
    overlay.querySelector("[data-mtc-db-import]").addEventListener("click",() => fileInput.click());
    fileInput.addEventListener("change",() => window.importDatabaseCorrectionsFromFile(fileInput));
    return overlay;
  }

  window.openDatabaseEditorDialog = function(){
    if(!ADMIN_MODE) return;
    const overlay = ensureDatabaseDialog();
    overlay.hidden = false;
    document.documentElement.classList.add("mtc-db-dialog-open");
  };

  function closeDatabaseEditorDialog(){
    const overlay = document.getElementById("mtcDatabaseEditorOverlay");
    if(overlay) overlay.hidden = true;
    document.documentElement.classList.remove("mtc-db-dialog-open");
  }
  window.closeDatabaseEditorDialog = closeDatabaseEditorDialog;

  function ensureDatabaseButton(){
    if(!ADMIN_MODE) return;
    if(document.getElementById("databaseEditorButton")) return;
    const tools = document.querySelector("#footerTitle .footer-icon-tools");
    if(!tools) return;
    const button = document.createElement("button");
    button.type = "button";
    button.id = "databaseEditorButton";
    button.className = "mtc-db-footer-button";
    button.textContent = "🗃️";
    button.title = "Exporter ou importer une base corrigée";
    button.setAttribute("aria-label","Exporter ou importer une base corrigée");
    button.addEventListener("click",window.openDatabaseEditorDialog);
    tools.insertBefore(button,tools.firstChild);
  }

  /* Les anciens éditeurs restent compatibles avec l’export complet. */
  document.addEventListener("pharma-herb-edited",event => {
    const id = event.detail && event.detail.herbId ? String(event.detail.herbId) : "";
    const field = event.detail && event.detail.field ? String(event.detail.field) : "";
    if(!id || !field || field === "image" || field === "database" || field === "database-import") return;
    const target = pharmaMap()[id];
    const prefix = LEGACY_PREFIXES.pharmacology[field];
  ...
  document.addEventListener("acu-point-edited",event => {
    const id = event.detail && event.detail.point ? String(event.detail.point) : "";
    const field = event.detail && event.detail.field ? String(event.detail.field) : "";
    if(!id || !field || field === "image" || field === "database" || field === "database-import") return;
    const target = acuMap()[id];
    const prefix = LEGACY_PREFIXES.acupuncture[field];
    if(!target || !prefix) return;
    try{
      const value = localStorage.getItem(prefix + id);
      if(value !== null) target[field] = value;
  ...
  document.addEventListener("input",event => {
    const note = event.target && event.target.closest && event.target.closest(".point-note-textarea[data-point]");
    if(note){
      const id = String(note.dataset.point || "");
      if(acuMap()[id]) acuMap()[id].notes = note.value;
    }
    const acu = event.target && event.target.closest && event.target.closest("[data-acu-compare-edit]");
    if(acu){
      const id = acu.dataset.acuPointId || "";
      const field = acu.dataset.acuCompareEdit || "";
      const target = acuMap()[id];
      if(target && field) target[field] = String(acu.innerText || "").replace(/\u00a0/g," ").trim();
    }
    const pharma = event.target && event.target.closest && event.target.closest("[data-pharma-compare-edit]");
    if(pharma){
      const id = pharma.dataset.pharmaHerbId || "";
      const field = pharma.dataset.pharmaCompareEdit || "";
      const target = pharmaMap()[id];
      if(target && field) target[field] = String(pharma.innerText || "").replace(/\u00a0/g," ").trim();
    }
  });

  applyAllOverrides();
  if(document.readyState === "loading") document.addEventListener("DOMContentLoaded",ensureDatabaseButton);
  else ensureDatabaseButton();

  window.mtcDatabaseEditor = {
    saveRecord,
    getOverrides:() => clone(overrides),
    getOfficialRecord:(domain,id) => clone(recordMaps(domain).official[id] || null)
  };
})();
